...
{
    public class BackpackableProps
    {
    ...
    // I couldn't override the one method I needed, so this is the solution
    public class CollectibleBehaviorBetterBag : CollectibleBehavior, IHeldBag, IAttachedInteractions
    {
        ...
        public readonly string[] ExcludedAttributes = { "bagsize", "slotcolor" };

        public CollectibleBehaviorBetterBag(CollectibleObject collObj) : base(collObj)
        {
        }
        ...
        public void Store(ItemStack bagstack, ItemSlotBagContent slot)
        {
            ...
            {
                SetAttributesFromStacks(bagstack, slot, slot.Empty);
            }
        ...
        public virtual void SetAttributesFromStacks(ItemStack bag, ItemSlotBagContent bagSlot, bool stackRemoved)
        {
            InventoryPlayerBackPacks backpack = bagSlot.Inventory as InventoryPlayerBackPacks;
            Dictionary<string, BackpackableProps> propsBySlot = new();
            BackpackableProps props;
            ITreeAttribute attrs;
            foreach (var slot in backpack)
            {
                if (slot.Empty) continue;
                if (slot.Itemstack.Collectible.Attributes.KeyExists("backpackableProps"))
                {
                    props = slot.Itemstack.Collectible.Attributes["backpackableProps"].AsObject<BackpackableProps>();
                    if (props == null) continue;

                    if (props.Codes.Count == 0 || props.Slot == null) continue;

                    if (!propsBySlot.ContainsKey(props.Slot))
                    {
                        propsBySlot.Add(props.Slot, props);
                    }
                    else
                    {
                        if (propsBySlot[props.Slot].Priority > props.Priority)
                        {
                            propsBySlot[props.Slot] = props;
                        }
                    }
                }
            }

            attrs = bag.Attributes.GetOrAddTreeAttribute("types");
            foreach (var attr in attrs)
            {
                for (int i = 0; i < ExcludedAttributes.Length; i++)
                {
                    if (attr.Key == ExcludedAttributes[i]) continue;
                }
                attrs.SetString(attr.Key, "none");
            }
            foreach (var entry in propsBySlot)
            {
                foreach (var code in entry.Value.Codes)
                {
                    attrs.SetString(code.Key, code.Value);
                }
            }

            var playerInventory = bagSlot.Inventory as InventoryBasePlayer;
            if (playerInventory != null)
            {
                playerInventory.Owner.MarkShapeModified();
            }
        }
